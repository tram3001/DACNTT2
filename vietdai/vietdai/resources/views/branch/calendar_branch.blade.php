                   ...
                   <div class="row">
                    <div class="col-lg-12" >
                      <div style="float: right">
                        <button id="btn"  style="border:none;background:none"><i class="fi fi-rr-camera-viewfinder" style="font-size: 20px"></i></button>
                      </div>
                    </div>
                    <div class="col-lg-9" style="padding-top:10px;padding-bottom:10px">
                      <h5>LỊCH TRONG TUẦN CỦA CHI NHÁNH {{$branch[0]->name}}</h5>
                      <a  href="/general_calendar/branch={{$branch[0]->id}}" style="font-size: 15px">Xem lịch chi nhánh tổng quát</a><br>
                      <a  href="/calendar_branch/branch={{$branch[0]->id}}" style="font-size: 15px">Tuần hiện tại</a>
                      ...
                      @if (Auth::user()->type==0)
                        <select name="" id="foo" class="form-control" style="width:100%">
                          ...
                          @foreach ($list_branch as $item)                           
                              <option value="/calendar_branch/branch={{$item->id}}" type="submit"><a href="#">{{$item->name}}</a></option>
                          @endforeach
                        </select>
                      @endif
                    </div>
                    ...
                    <div class="col-lg-3">
                      <div style="float: left">
                        <form action="/calendar_branch/week" method="get">
                          @csrf
                          ...
                          <thead style="background-color: #526484;padding:auto" >
                            ...
                            <?php
                              for($t=2;$t<9;$t++){
                                ...
                                ?>
                                  <th style="color: white">{{$k}}<br><a style="color: white; font-size:12px">{{$days[$t-2]->format('d-m')}}</a></th>
                                <?php
                              }
                            ?>
                          </thead>
                                         ...
                                         <?php
                                            ...
                                            <?php
                                              ...
                                              foreach($teacher as $item){
                                                $c=0;
                                                $a=strpos($item->calendar, $key );
                                                if($a!=''){
                                                  $class=DB::table('class')->whereRaw('teacher=?',$item->id)->get()->toArray();
                                                  if(count($class)){
                                                    foreach ($class as $kt) {
                                                      # code...
                                                      $b=strpos($kt->calendar, $key );
                                                      if ($b!='') {$day=$days[$j-2];$day=$day->format('Y-m-d');
                                                        if(strtotime($kt->bd)<=strtotime($day)&& strtotime($kt->kt)>=strtotime($day) ){
                                                          $c=1;
                                                          ?>
