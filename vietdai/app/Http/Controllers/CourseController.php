...
class CourseController extends Controller
{
    ...
    public function search_course(Request $request){
        $course=DB::table('course')->where('id','Like','%'.$request->search.'%')->orWhere('language','Like','%'.$request->search.'%')->orWhere('name','Like','%'.$request->search.'%')->orWhere('id','Like','%'.$request->search.'%')->paginate(10);
        ...
        if(Auth::user()->type==0){
            ...
            if( $request->form=='all' && $request->status=='all'){
                ...
                if($request->language!='all'){
                    ...
                    $class=$class->whereRaw('ht=?',$request->language);
                }
    ...
    public function list_class_staff($id){
        if(Auth::user()->type==0){
            $class=DB::table('class')->whereRaw('teacher=?',$id)->paginate(10);
            ...
            $class=DB::table('class')->whereRaw('teacher=?',$teacher[0]->id)->paginate(10);
        }if(Auth::user()->type==2){
