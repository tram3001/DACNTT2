...
class BranchController extends Controller
{
    ...
    public function create_branch(Request $request){
        ...
        DB::insert("INSERT INTO users (name,password,change_pass,branch,type) VALUES(:name,:password,:change_pass,:branch,:type)",[
            ":name"=>$name_account,
    ...
    public function delete_branch(Request $request){
        ...
        DB::table('users')->where('branch',$request->id)->delete();
    ...
    public function insert_staff(Request $request){
        ...
        DB::insert("INSERT INTO users (name,password,change_pass,branch,type,staff) VALUES(:name,:password,:change_pass,:branch,:type,:staff)",[
            ":name"=>$request->email,
    ...
    public function calendar_week(Request $request){
        ...
        if(Auth::user()->type==0){
            ...
            $staff=DB::table('staff')->whereRaw('id=?',$request->id)->whereRaw('id_branch=?',Auth::user()->branch)->get()->toArray();     
            $teacher=$staff[0];
        }if(Auth::user()->type==2){
    ...
    public function filter_staff(Request $request){
        if(Auth::user()->type==0){
            ...
            $list=$staff->orderByDesc('id')->paginate(10);
    ...
    public function search_staff(Request $request){
        if(Auth::user()->type==0){
            ...
            if($request->language!='all' &&  $request->language!=''){
                $staff=$staff->where('languages','like','%'.$request->language.'%');
            }
            if($request->form_work!='all' &&  $request->form_work!=''){
                $staff=$staff->where('form_work','like','%'.$request->form_work.'%');
            }
            ...
            { $query->where('name','like','%'.$search.'%')->orWhere('id','like','%'.$search.'%')
                ...
                ->orWhere('sex','like','%'.$search.'%')->orWhere('cccd','like','%'.$search.'%')->orWhere('email','like','%'.$search.'%')
                ->orWhere('address1','like','%'.$search.'%')->orWhere('languages','like','%'.$search.'%');})->orderByDesc('id')->paginate(10);
    ...
    public function hide_teacher(Request $request){
        ...
        DB::statement("DELETE from users where staff=?",[$request->id]);
        return back();
    }
    ...
    public function list_hide_teacher(){
        ...
        return view('staff.staff_hide',compact('list','list_branch'))->with('i',(request()->input('page',1)-1)*10);
    }
    //display teacher
    ...
    public function delete_teacher(Request $request){
        $class=DB::table('class')->whereRaw('teacher=?',$request->id)->get()->toArray();
        foreach($class as $row){
            DB::statement("DELETE from list_class where class=?",[$row->id]);
        }
        DB::table('users')->where('staff',$request->id)->delete();
