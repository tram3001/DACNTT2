...
class StudentController extends Controller
{
    ...
    {   
        ...
        DB::insert("INSERT INTO list_class (class,student) VALUES (:class,:student)",[
            ":class"=>$request->class,
    ...
    public function search(Request $request){
        ...
        $branch=DB::table('branch')->whereRaw('id=?',$class[0]->branch)->get()->toArray();
        $course=DB::table('course')->whereRaw('id=?',$class[0]->course)->get()->toArray();
        return view('class.detail_class',compact('today','list_student','count','class','teacher','branch','course'));
    ...
    public function search_addstudent(Request $request){
        ...
        if(Auth::user()->type==0){
            ...
            $list_student=$list_student->where('name','like','%'.$request->search.'%')->orWhere('sex','like','%'.$request->search.'%')->orWhere('phone','like','%'.$request->search.'%')->orWhere('nationality','like','%'.$request->search.'%')->orderByDesc('id')->paginate(10);
        }
        else{
            $class=DB::table('class')->whereRaw('id=?',$request->class)->whereRaw('branch=?',Auth::user()->branch)->get()->toArray();
            $student=DB::table('student')->whereRaw('branch=?',Auth::user()->branch);
            $list_student=$student->where(function($query) use ($search)
            { $query->where('name','like','%'.$search.'%')->orWhere('phone','like','%'.$search.'%')->orWhere('sex','like','%'.$search.'%')->orWhere('nationality','like','%'.$search.'%');})->orderByDesc('id')->paginate(10);
    ...
    public function delete_student_class(Request $request){
        DB::statement("DELETE from list_class where student=? and class=?",[$request->id,$request->class]);
        return back();
    }
}
